...
document.querySelectorAll('th[data-sort]').forEach(th => {
  th.addEventListener('click', () => {
    const i = th.cellIndex, table = th.closest('table'), body = table.tBodies[0], rows = [...body.rows];
    const asc = !th.classList.contains('asc');
    table.querySelectorAll('th').forEach(h => h.classList.remove('asc', 'desc'));
    th.classList.add(asc ? 'asc' : 'desc');
    // leading number wins ("2.96 conf 0.96" sorts by 2.96, "—" sorts last); otherwise case-insensitive text
    const val = r => { const t = r.cells[i].textContent.trim().replace(/,/g, ''); const m = t.match(/^-?\d+(\.\d+)?/); return m ? parseFloat(m[0]) : t === '—' || t === '' ? (asc ? Infinity : -Infinity) : t.toLowerCase(); };
